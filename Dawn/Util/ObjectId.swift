...
class ObjectId {
    ...
    init?(mongoId: String?) {
        ...
        NSScanner(string: timestampHex).scanHexInt(&timestampInt)
        NSScanner(string: machineHex).scanHexInt(&machineInt)
        NSScanner(string: pidHex).scanHexInt(&pidInt)
        NSScanner(string: incrementHex).scanHexInt(&incrementInt)
